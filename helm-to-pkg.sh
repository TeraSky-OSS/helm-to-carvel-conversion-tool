...
[Optional Flags]
  ...
  --output-dir : (default: /tmp/carvel-helm-packages) - DONT SET WHEN RUNNING IN CONTAINER
  ...
  --helm-repo-url : The Helm Repo URL. - (default: https://charts.bitnami.com/bitnami)
  --helm-repo-name : The local Helm Repo name. - (default: bitnami)
  --add-helm-repo : A boolean flag to enable adding a non existent helm repo. (default: false)
  --package-domain-suffix : Package names require a naming convention of <APP NAME>.x.y . This parameter allows configuring the x.y suffix. - (default: helm.charts)
...
complete -F _helm-to-pkg helm-to-pkg
EOF
    exit 1
  else
...
if [[ $temp_dir ]]; then
  ...
  mkdir -p /tmp/helm-to-pkg/helm
  mkdir -p /tmp/helm-to-pkg/helm/lock-files
  temp_dir="/tmp/helm-to-pkg/helm"
fi

if [[ $output_dir ]]; then
  mkdir -p $output_dir/packages
else
  mkdir -p /tmp/carvel-helm-packages/packages
  output_dir="/tmp/carvel-helm-packages"
fi
...
if [[ $package_domain_suffix ]]; then
  ...
  package_domain_suffix="helm.charts"
  echo "Packages will be generated with the naming convention <CHART NAME>.$package_domain_suffix"
fi
...
spec:
  ...
  longDescription: "$helm_repo_name - ${appFolder} Helm Chart"
...
EOF
          ...
          sed -i 's/^\"\//"/g' $temp_dir/$helm_repo_name/$appFolder/$versionFolder/images.list.tmp
	  ...
	  if [[ -d "${temp_dir}/${helm_repo_name}/${appFolder}/${versionFolder}/${appFolder}/charts" ]]; then
	    cd $temp_dir/$helm_repo_name/$appFolder/$versionFolder/$appFolder/charts
	      ...
	      sed -i 's/^\"\//"/g' $temp_dir/$helm_repo_name/$appFolder/$versionFolder/$d/images.list.tmp
	  ...
	  cp -r $temp_dir/$helm_repo_name/$appFolder/$versionFolder/$appFolder/* $output_dir/packages/$appFolder.$package_domain_suffix/$versionFolder/config/
          ...
          readme-generator -v $appFolder/values.yaml --metadata values-schema.json 1>/dev/null 2>/dev/null
	      ...
	      echo "Values File does not comply with bitnami standards. Generating Manifest with helm schema-gen as a fallback - this will provide a less deatiled openapi schema"
	      helm schema-gen $appFolder/values.yaml > values-schema.json
	      json2yaml values-schema.json  > temp.yaml
              sed -i 's|^  |      |g' temp.yaml
              tail -n +3 temp.yaml > values-schema.yaml
	      sed -i '1s/^/      title: "Chart Values"\n/'
            fi
          else
            echo "Values File does not comply with bitnami standards. Generating Manifest with helm schema-gen as a fallback - this will provide a less deatiled openapi schema"
            helm schema-gen $appFolder/values.yaml > values-schema.json
